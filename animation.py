...
class SolidObjectVisualizer:
    ...
    def update(self, frame, ax, forces, torques):    
        #for j in range(steps):
            ...
            for _, solid_obj in enumerate(self.solid_objs):
                ...
                artists.append(ax.scatter(centroid[0], centroid[1], centroid[2], 
                                          color='r', s=50, label="Centroid" if i == 0 else None))
            ...
            for _, ball_obj in enumerate(self.ball_objs):
                ...
                # Plot the centroid
                artists.append(ax.scatter(centroid[0], centroid[1], centroid[2], color='r', 
                                          s=50, label="Centroid" if i == 0 else None))
    ...
    def visualize(self, steps=1000, forces=None, torques=None):
        ...
        # 设置输出视频的文件名和格式（使用FFmpeg作为MovieWriter）
        ani.save('newton.mp4', writer='ffmpeg', fps=30)
        plt.show()
